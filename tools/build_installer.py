...
def product_string(name: str) -> str:
    """One string from ``kurukuru/product.py``, read rather than imported.

    Same reasoning as :func:`version`: importing the backend would make the
    build depend on it being installed into the interpreter running the build.
    """
    import re
    ...
    if not match:
        raise BuildError(f"Could not read {name} from backend/kurukuru/product.py")
    return match.group(1)


def installer_define(name: str) -> str:
    """One ``#define`` from ``packaging/windows/kurukuru.iss``.

    The publisher lives there, because that is the file Inno reads to stamp the
    installer. Reading it rather than repeating it is what makes the frozen
    executable and the installer agree — which is the consistency SignPath
    checks for across the artifacts in one release.
    """
    import re

    text = (REPO / "packaging" / "windows" / "kurukuru.iss").read_text(encoding="utf-8")
    match = re.search(rf'^#define\s+{name}\s+"([^"]+)"', text, re.M)
    if not match:
        raise BuildError(f"Could not read #define {name} from kurukuru.iss")
    return match.group(1)


def write_version_resource(out: Path) -> Path:
    """A Win32 VERSIONINFO resource for the frozen executable.

    **PyInstaller gives an executable no version resource unless it is handed
    one**, and this build never handed it one — so `kurukuru.exe` shipped in
    0.1.0, 0.1.1 and 0.1.2 with an empty ProductName, ProductVersion and
    FileVersion. Windows shows a blank Details tab for it, and anything that
    inventories software on a machine sees an anonymous binary.

    That was invisible because nothing looks at it: the installer carries its
    own metadata (Inno writes it from AppName and AppVersion), the dashboard
    reports the version over HTTP, and `kurukuru version` reads the installed
    package. Code signing is what surfaced it — SignPath requires a matching
    ProductName and a consistent ProductVersion on every signed artifact, and
    an empty field is not a match.

    Generated from ``product.py`` rather than written out here, so the frozen
    executable, the installer, ``/health`` and ``kurukuru version`` cannot
    disagree about what version they are.
    """
    release = version()
    name = product_string("PRODUCT_NAME")
    tagline = product_string("PRODUCT_TAGLINE")
    publisher = installer_define("AppPublisher")

    # Win32 wants four integers. A release is "0.1.2"; a working tree may be
    # "0.0.0-dev", so only the leading numeric parts are taken and the rest is
    # padded. The *string* fields keep the full version, including any suffix,
    # because that is the one a human reads.
    import re

    numbers = [int(part) for part in re.findall(r"\d+", release.split("-")[0])][:4]
    numbers += [0] * (4 - len(numbers))
    quad = ", ".join(str(n) for n in numbers)

    resource = out / "version-info.txt"
    resource.write_text(
        f"""# Generated by tools/build_installer.py. Do not edit.
VSVersionInfo(
  ffi=FixedFileInfo(
    filevers=({quad}),
    prodvers=({quad}),
    ...
    StringFileInfo([
      StringTable(
        ...
        [
          ...
          StringStruct('FileDescription', {f"{name} - {tagline}"!r}),
          StringStruct('FileVersion', {release!r}),
          StringStruct('InternalName', 'kurukuru'),
          StringStruct('LegalCopyright', 'Apache-2.0. See LICENSE.'),
          StringStruct('OriginalFilename', 'kurukuru.exe'),
          StringStruct('ProductName', {name!r}),
          StringStruct('ProductVersion', {release!r}),
        ],
      )
    ]),
...
def freeze_backend(out: Path, work: Path) -> Path:
    ...
    run(
        [
            ...
            "--workpath", str(work),
            "--specpath", str(work),
            # Without this the executable has no version resource at all:
