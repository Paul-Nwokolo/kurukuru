...
recorded inside runtime files and database rows. :mod:`app.state_migration` and
...
#: :mod:`app.config` re-exports it as ``DEFAULT_STATE_DIR`` and roots every
#: other path under it.
STATE_DIR = "~/.kurukuru"
...
#: 16 they each spelled it independently — ``app.auth`` and ``app.cli.client``,
...
#: ``app.auth`` (it is an API client, not part of the control plane), so the one
...
#: now. :mod:`app.state_migration` moves this to :data:`STATE_DIR` once.
