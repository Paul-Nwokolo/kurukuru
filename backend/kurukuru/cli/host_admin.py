...
Everything else in ``kurukuru/cli`` goes over HTTP, and ``test_the_cli_never_reaches
...
def _session():
    ...
    ``kurukuru.database.engine`` is read at call time rather than bound at import.
    That is not style: the module-level engine is created from settings when the
    package is first imported, so a `from kurukuru.database import engine` here would
...
def validate_password(password: str) -> str:
    ...
    from kurukuru.models import validate_password as _validate
